...
highlight const_def ctermfg=DarkMagenta ctermbg=Black
highlight variable_def ctermfg=Darkcyan ctermbg=Black
...
highlight tic ctermfg=DarkCyan ctermbg=Black

syntax match compile /[^][]/
syntax match execute /[^][]/
syntax match tic /` [^ ]\+/
syntax match define /: [^ ]\+/
syntax match const_def /' [^ ]\+/
syntax match variable_def /& [^ ]\+/
syntax match error / then ;/

syntax region brackets start=":\|]"hs=s+ end="\[" contains=variable_def,tic,define,compile,const_def,error
